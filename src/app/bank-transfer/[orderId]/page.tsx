...
export async function generateMetadata({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const label = product?.bankTransfer.productLabel ?? 'плащане';
  return {
    title: `Банков превод · ${label} | Coaching Real`,
...
export default async function BankTransferPage({ params }: { params: Promise<{ orderId: string }> }) {
  ...
  const product: Product | null = getProduct(order.product);
  if (!product) notFound();

  const reference = `${product.bankTransfer.referencePrefix} ${order.name} ${shortCode(orderId)}`;
        ...
        >
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <p style={{ fontSize: '15px', color: 'rgba(0,0,0,0.6)', lineHeight: 1.7, margin: 0 }}>
              {order.name}, за да потвърдим поръчката ти, направи превод по сметката по-долу.
              Веднага щом получим плащането (1–2 работни дни), ще ти изпратим имейл с
              достъп и всички детайли.
            </p>
          </div>
          ...
          >
            <strong style={{ color: '#4a3000' }}>Важно:</strong> Сумата за превод е <strong>{amountDisplay}</strong>.
            Веднага щом получим плащането (1–2 работни дни), ще ти изпратим имейл с
            достъп до <strong>{product.bankTransfer.productLabel}</strong>.
          </div>
          ...
          >
            ...
            <ol style={{ margin: 0, padding: '0 0 0 20px', fontSize: '14px', color: 'rgba(0,0,0,0.7)', lineHeight: 1.8 }}>
              ...
              <li>{product.bankTransfer.nextStepCopy}</li>
            </ol>
          </div>
